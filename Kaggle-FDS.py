...
#bestAlpha = pd.Series(bestAlpha, index = alphas)
#prices = pd.DataFrame({"price":y, "log":ylog})
#
#plt.figure(1)
#f, axarr = plt.subplots(2, 2, figsize=(10, 9))
#axarr[0, 0].hist(prices['price'])
#axarr[0, 0].set_title('Fig1. SalePrice')
#axarr[0, 1].hist(prices['log'])
#axarr[0, 1].set_title('Log(SalePrice)')
#axarr[1, 0].scatter(train['GrLivArea'], train['SalePrice'])
#axarr[1, 0].set_title('Fig2. Outliers')
#axarr[1, 1].plot(alphas, bestAlpha.values)
#axarr[1, 1].set_title('Fig3. Best alpha')
#plt.tight_layout()
#plt.show()
#
#coef = pd.Series(model_lasso.coef_, index = trainnum1.columns)
#mainCoef = pd.concat([coef.sort_values().head(10),
#                     coef.sort_values().tail(10)])
#figFour = mainCoef.plot(kind = "barh", title="Fig4. Lasso selected features vs Influence")
